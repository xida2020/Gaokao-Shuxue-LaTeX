...
\end{center}
\vspace{-2em}
\qs 如图，在平行四边形$ ABCD $中，$ AC,BD $相交于点$ O,E $为线段$ AO $的中点，若$ \vv{BE}=\lambda\vv{BA}+\mu\vv{BD}\left(\lambda,\mu\inR \right) $，则$ \lambda+\mu $=\tk.
\begin{center}
\begin{tikzpicture}
\tikzmath{
	\a =cos(60);
...
\node[coordinate,label=left:$B$](B) at(0,0){};
\node[coordinate,label=above left:$A$](A) at($(2*\a,2*\b)$){$A$};
\node [coordinate,label=right:$C$](C) at($(B)+(4,0)$){$C$};
\node [coordinate,label=above right:$D$](D) at($(A)+(4,0)$){$D$};
\draw (B)--(A)--(D)--(C)--cycle;
\draw[name path=l1] (A)--(C);
\draw[name path=l2] (B)--(D);
\node[coordinate,label=above:$E$](E) at($(A)!0.25!(C)$){$E$};
\draw(B)--(E);
%intersection
\path[name intersections={of=l1 and l2}](intersection-1) node[below]{$O$};
\end{tikzpicture}	
\end{center}
