...
\node[anchor=west] at (0.25,\wholeboxht-1em) {\textbf{题号:}};
\node[anchor=west] at (0.25,\summht-1em){\textbf{本章总结:}};
\node[anchor=west,fill=white] at (\cuewd+1em,\wholeboxht-1em) {\textbf{\ 学习体验:\ }};
\node[anchor=west] at (0.25,\wholeboxht+2em){%
    \parbox[t]{\cuewd}{姓名:\par\smallskip 日期:}%
};
\end{tikzpicture}
\end{center}
\end{document}