...
\subsection{数列与函数}
%在函数的意义下，数列是定义域为正整数集$ \mathbb{N^*} $(或它的有限子集$ \left\{1,2,3,\cdots,n\right\} $)的特殊函数，数列的通项公式就是相应的函数解析式，即$ a_n=f(n)~(n\in\mathbb{N^*}) $.
\subsection{通项公式}
\subsubsection{通项公式}
...
\begin{parts}
\part 若$a_1=1$，求数列$\{a_n\}$的通项公式；
\part 求数列$\{a_n\}$唯一，求$a$的值.
\end{parts}
\kongbai
...
%  例2.4 p60
\qs 设$\{a_n\}$是公比为$q$的等比数列.
\begin{parts}
\part 求$\{a_n\}$的前$n$项和公式；
\part 设$q\neq 1$，证明数列$\{a_n+1\}$不是等比数列.
\end{parts}
\newpage

%  练习2.6 p61
\qs 对于给定的正整数$k$，若数列$\{a_n\}$满足：$a_{n-k}+a_{n-k+1}+\cdots+a_{n-1}+a_n+\cdots +a_{n+k-1}+a_{n+k}=2ka_n$对任意正整数$n(n>k)$总成立，则称数列$\{a_n\}$是“$P(k)$数列”.
\begin{parts}
\part 证明：等差数列$\{a_n\}$是“$P(3)$数列”；
\part 若数列$\{a_n\}$既是“$P(2)$数列”，又是“$P(3)$数列”，证明：$\{a_n\}$是等差数列.
\end{parts}
\kongbai


%  练习2.7 p62
\qs  成等差数列的三个正数的和等于$15$，并且这三个数分别加上$2,~5~13$后成为等比数列$\{b_n\}$中的$b_3,~b_4,~b_5$.
\begin{parts}
\part 求$\{b_n\}$的通项公式；
\part 数列$\{b_n\}$的前$n$项和为$S_n$，求证$\big\{S_n+\dfrac54\big\}$是等比数列.
\end{parts}
\kongbai

%  练习2.8 p62
\qs  记$S_n$为等比数列$\{a_n\}$的前$n$项和，已知$S_2=2,~S_3=-6$.
\begin{parts}
\part 求$\{a_n\}$的通项公式；
\part 求$S_n$，并判断$S_{n+1},~S_{n},~S_{n+2}$是否成等差数列.
\end{parts}
\kongbai
...
\begin{parts}
\part 求数列$\{a_n\}$的公比；
\part 证明：对任意$k\in \mathbb{N}^*,S_{k+2},~S_{k},~S_{k+1}$成等差数列.
\end{parts}
\kongbai
...
\begin{parts}
\part 证明：$a_{n+2}-a_n=\lambda$；
\part 证明：是否存在$\lambda$，使得$\{a_n\}$为等差数列？并说明理由.
\end{parts}
\kongbai
...
\begin{parts}
\part 求数列$\{a_n\}$的通项公式；
\part 若存在$k\in \mathbb{N}^*$，使得$S_{k+1},S_{k},S_{k+2}$成等差数列，试判断：对于对于任意的$m\in \mathbb{N}^*$，且$m\geqslant 2,~a_{m+1},a_m,a_{m+2}$是否成等差数列？证明你的结论.
\end{parts}
\kongbai
...
\begin{parts}
\part 若$c=0$，且$b_1,~b_2,~b_4$成等比数列，证明：$S_{nk}=n^2S_k~(k,n\in \mathbb{N}^*)$；
\part 若$\{b_n\}$是等差数列，证明：$c=0$.
\end{parts}
\kongbai


%  练习2.11 p66
\qs 已知$\{x_n\}$是各项均为正数的等比数列，且$x_1+x_2=3,~x_3-x_2=2$.
\begin{parts}
\part 求数列$\{x_n\}$的通项公式；
\part 如图所示，在平面直角坐标系$xOy$中，依次连接点$P_1(x_1,1),~P_2(x_2,2),~\cdots,~P_{n+1}(x_{n+1},n+1)$得到折线$P_1P_2\cdots P_{n+1}$，求由该折线与直线$y=0,~x=x_i~(x\in \{x_n\})$所围成的区域的面积$T_n$. 
\end{parts}
\kongbai
...
\begin{parts}
\part 求数列$\{a_n\}$的通项公式；
\part 令$b_n=a_n3^n~(x\in \mathbb{R})$，求数列$\{b_n\}$前$n$项和的公式.
\end{parts}
\kongbai
...
\begin{parts}
\part 求数列$ \left\{a_n\right\} $的通项公式；
\part 设$ {b_n}=(n+1)a_n $，$ T_n $是数列$ {b_n} $的前$ n $项和，$ n\in\mathbb{N^*} $,求$ T_n $.
\end{parts}
\kongbai
\qs 已知等差数列$\{a_n\}$满足$ a_1+a_2=10 $,~$a_4-a_3=2$.
\begin{parts}
...
\kongbai
\question
$S_n$为数列$\{a_n\}$的前$n$项和，已知$a_n>0$，$a_n^2+2a_n=4S_n+3$，其中$n\in \mathbb{N}^*$.
\begin{parts}
\part 求$\{a_n\}$的通项公式；
